...
import CPA_TEAM_11.DynamicBeat;
		...
		while (!ds.isClosed()) {
			...
			try {
				...
				case "score":	// 상대방 스코어 받아서 셋팅
					...
					break;
				case "note1":
					System.out.println(str[1] + " n1");
					if (str[1].equals("D1")) {
						GameFrame.game.d1 = true;
						System.out.println(GameFrame.game.d1);	
					} else if(str[1].equals("F1")) {
						GameFrame.game.f1 = true;
						System.out.println(GameFrame.game.f1);
					} else if(str[1].equals("J1")) {
						GameFrame.game.j1 = true;	
						System.out.println(GameFrame.game.j1);
					} else if(str[1].equals("K1")) {
						GameFrame.game.k1 = true;
						System.out.println(GameFrame.game.k1);
					}
				case "note2":
					System.out.println(str[1]  + " n2");
					if (str[1].equals("D2")) { 	
						System.out.println(GameFrame.game.d1);
						GameFrame.game.d1 = false;	
					} else if(str[1].equals("F2")) {
						GameFrame.game.f1 = false;
					} else if(str[1].equals("J2")) {
						GameFrame.game.j1 = false;	
					} else if(str[1].equals("K2")) {
						GameFrame.game.k1 = false;
					}
				case "Perfect":
					System.out.println("클라이언트에서 퍼펙트");
					Note.Otherp = true;
					try {
						Thread.sleep(5);
						Note.Otherp = false;
...
//					new DynamicBeat();
	...
	public void sendChangeNote1(String note) {
		synchronized (oos) {
			try {
				oos.writeObject("note1#" + note );
			} catch (IOException e) {
				System.out.println(e.toString());
			}
		}
	...
	public void sendChangeNote2(String note) {
		synchronized (oos) {
			try {
				oos.writeObject("note2#" + note );
			} catch (IOException e) {
				System.out.println(e.toString());
			}
		}
	}
	public void sendNoteJudge(String note) {
		synchronized (oos) {
			try {
				oos.writeObject("judge#" + note );
			} catch (IOException e) {
				System.out.println(e.toString());
			}
		}
